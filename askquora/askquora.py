...
def parse_args():
	parser = argparse.ArgumentParser(description='Quora Q&A right from the command-line')
	parser.add_argument('query', metavar='QUERY', type=str, nargs='*',
	                    help='the question to find the answer of')
	parser.add_argument('-n', '--no-color', help='do not colorize or style text',
	                    action='store_true')
	return parser
...
# check if the link is a quora question or something else
...
# display the answer to the repected question
def answer_question(link, colored=False):
	...
	try:
		answer = soup.find('div', {'class':'ExpandedQText ExpandedAnswer'}).get_text()
	except AttributeError:
		exit('Sorry, this question has not been answered yet..')
...
if __name__ == '__main__':
	...
	# using sys.args[1:] to not pass filename as positional argument
	raw_args = sys.argv[1:]
